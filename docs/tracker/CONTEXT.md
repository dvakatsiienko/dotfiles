...
- **coordinator** — the session that owns tracker work: creates, updates, closes, triages, and never writes product code. **`cclio`** holds the role since 2026-08-21 (DOT-188) — a ccli session in `~/projects/dotfiles/cclio`. **`dpatch`** is the desktop dispatch surface it took the role from, and the two names are never interchangeable.
