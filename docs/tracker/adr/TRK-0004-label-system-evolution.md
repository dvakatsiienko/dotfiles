...
consequences: the role slot in `docs/tracker/CONTEXT.md` becomes `agent · human · needs human · needs agent · needs data`, and `docs/agents/triage-labels.md` maps the mattpocock `needs-info` role onto all three by direction. TRK-0002's L3 (`needs-info` retired) and L6 (no hyphens, workspace-level only) stand unchanged. ⚠️ the `standing` label's own description in linear still carries L4's old reasoning and wants a one-line edit — a tracker op, so dispatch's.
