---
title: cw memory snapshot — 2026-08-31
captured: 2026-08-31T09:32Z
supersedes-snapshot: cw-memory-snapshot-2026-08-29.md (145 lines)
lineage: 2026-08-29 anchor → verified 08-30 → verified + refreshed 08-31 (146 lines)
verified: 2026-08-31T06:20Z — cwrk scheduled probe, all 12 entries, md5 per entry, one human edit, zero drift
captured-by: cwrk (live thread with dima, refreshed on his ask after the scheduled probe reported)
...
**what changed from the 08-29 anchor.** exactly one line, in `/topics/obsidian.md`:

```text
  - [stated] 2026-08: considering scrapping notion entirely (incl. recipes) in favour of obsidian — testing the vault for it; notion mcp editing is atrocious
```

added by dima in a live session on 2026-08-30T12:39Z. all 16 of that entry's anchor lines survived
byte-identical and in order; the other 11 entries md5-matched outright. **145/145 → 146 lines.**

read `## 6` for the standing conclusions before re-opening any of this.

---

## 1. listing — every entry at capture

sizes and mtimes refreshed 2026-08-31T09:32Z (`memory_list`); bodies below reflect that state.
...
- `/topics/obsidian.md` — 2371 bytes — 2026-08-30T12:39:20Z  ← only entry changed since the 08-29 anchor
...
awk -v pat="### /areas/fleet.md" '$0==pat{f=1;next} f&&/^### /{exit} f' \
  cw-memory-snapshot-2026-08-31.md | grep '^- \[stated\]' | md5sum
```
...
md5 of the entry's `- [stated]` lines only, in file order, as of 2026-08-31T09:32Z. 146 lines total.
...
- `/topics/obsidian.md` — 17 lines — `3d38120c4ba2c4d1b516833b25cad394`
...
- **2026-08-31** — full 12-entry sweep against the 08-29 anchor: **145/145 anchor lines
  byte-identical**, 11/12 entries md5-clean on first compare. `/topics/obsidian.md` differed by
  **one added line** — all 16 anchor lines survived in order, `+1` new. classified a **human edit**
  (dated fact, live decision), not drift. no rewording anywhere, no section or fact line lost.
  this file is the refreshed anchor at **146 lines**.

---

## 6. standing conclusions — read this before re-litigating the probe

three consecutive nights, escalating from 3 entries to all 12:

- **nothing silently regenerates `[stated]` lines.** the cc→cw bridge assumption holds; every
  observed change so far traced back to a human edit or a `prettify` pass.
- **only `[stated]` lines are diffable evidence.** frontmatter `description` and `/_router.md`
  `## the map` prose are prettify-owned and regenerable — never put load-bearing info there.
- **the probe's real cost is anchor staleness, not drift.** a legitimate human edit re-flags every
  night until the anchor is refreshed. refreshing is live-thread work, never a scheduled run.

**still open for cclio** — the `## known debt` section of `/_router.md` was present on 08-28 and
absent on 08-29. it predates every anchor and cannot be re-tested from here. if a `prettify` run
removed it, that violates `memory-update`'s own "zero data loss, shape only" contract, and the skill
needs a **scope guard: prettify never deletes a section, it collects it as a proposal**. two clean
full sweeps since have produced no second instance — the guard is still worth landing, but the
suspicion has not reproduced.
