...
Dima's Obsidian vault folder `/Users/dima/Library/Mobile Documents/iCloud~md~obsidian/Documents/Obsidian Dima's Vault/prompts/` is dpatch's personal email. `inbox.md` = raw dumps from Dima (mobile → iCloud sync). `worklog.md` = dpatch's processing copy with statuses.
