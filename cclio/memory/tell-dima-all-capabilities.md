...
**Why:** a surface could mount host directories the whole time via its own tool; it went
...
- On a new surface, state plainly what it has and what it lacks.
- 🚫 **Grading yourself is not allowed.** Supplying Dima the honest comparison is the job.
...
- **effort AND model per spawn**, both honoured
