...
⏰ 🦊👁️ github cached-commit purge — support ticket filed 2026-09-22 via the virtual agent for `dvakatsiienko/frame` commits `1cf21b6e…` and `4f749530…` (the two `.rayconfig` exports, purged from history the same day). check at every boot: `gh api repos/dvakatsiienko/frame/commits/1cf21b6 --jq .sha` → 404 means done, say so once and drop this line; still 200 after 3 days → dima nudges the ticket — set 2026-09-22
...
⏰ 🦊📜 frame migration backup — on/after 2026-09-24: `trash ~/frame-migration-backup` (pre-edit copies of every config the 09-23 rename touched, plus dima's three wip files). dima's yes is already given (2026-09-23); check first that the re-grants still hold (a chord line in `~/.local/share/x-monitor-hotkey-stats/` today) and that Claude Desktop's `x-cw` server is connected, then trash and drop this line — set 2026-09-23

⏰ 🦊👁️ vhs 0.12 writes no gif — [vhs #787](https://github.com/charmbracelet/vhs/issues/787): the render context is cancelled before the ffmpeg encode, exit 0, no file. our workaround: `assets/frame-link.tape` outputs frames, its header holds the ffmpeg command that builds the play-once gif. check at every brew upgrade of vhs: `gh issue view 787 -R charmbracelet/vhs --json state` → closed + a release carries it → the tape's `Output` goes back to `assets/frame-link.gif`, the header comment dies, drop this line. dima 2026-09-23: «keep an eye on it so we lift the vhs workaround» — set 2026-09-23
