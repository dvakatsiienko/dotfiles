...
def hero_sky(mode):
    stops = re.search(r'<linearGradient id="sk"[^>]*>((?:<stop[^>]*>)+)', (HERE / f'hero-grove-{mode}.svg').read_text()).group(1)
    return re.findall(r'stop-color="(#[0-9a-fA-F]{6})"', stops)
...
def sky(p, mode):
    top, bottom = hero_sky(mode)
    return (f'<defs><linearGradient id="{p}sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="{top}"/><stop offset="1" stop-color="{bottom}"/></linearGradient>'
            f'<clipPath id="{p}clip"><rect width="{W}" height="{H}" rx="12"/></clipPath>'
            f'<filter id="{p}paper" x="-10%" y="-20%" width="120%" height="160%"><feDropShadow dx="0" dy="1.5" stdDeviation="1.5" flood-color="#4a321a" flood-opacity=".22"/></filter></defs>'
...
FRAME = ('frame: a brass djinni lamp with gems, a flame and a curl of magic smoke', djinni)
