...
- `cw` shares the handoff store via the `handoff` MCP server (`~/projects/dotfiles/home/.claude/mcp-handoff-cw/`) — CSTs flow `cc`↔`cw` through `~/.claude/shelf/handoffs/`; the format is defined once in `CST-SPEC.md` next to the skills.
