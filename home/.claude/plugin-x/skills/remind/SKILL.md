...
Store at the stuck tier, **leading with the agent's signature emoji** plus an optional flavour
pick (`⏰📌 🦊👁️ …` — cclio-fable signs 🦊; another agent picks its own and keeps it stable).
Same lifecycle as `stick`.
