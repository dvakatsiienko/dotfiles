...
<!--
  ...
  copied, not symlinked: dima wants one install/update mechanism (the x marketplace), not a zoo;
  a clone in ~/projects felt clunky. refresh: re-fetch from the repo and re-apply this header.
-->
...
## Multi-lane verify (local extension, not upstream)
...
Detector scores are directional, never pass/fail truth: light editing swings every tool 15–30
points and false positives exist on all of them. Dima is the validator of record.
